...
export const MCP_TOOL_DEFINITIONS: { name: string; description: string; inputSchema: Record<string, unknown> }[] = [
  ...
  { name: "monitor_publish", description: "Publish one observation. An agent working for the monitor's owner names them in on_behalf_of and may then claim what the owner could; with nobody named, an agent may not assert tier 'src'. A refusal is itself a recorded observation.",
    inputSchema: obj({ id: S.id, bearer: S.bearer, type: { type: "string", description: "Reverse-DNS observation type." }, subject: { type: "string" },
      ...
      verification: { type: "string", enum: [...VERIFICATIONS] }, actor: { type: "string", description: "Who observed, e.g. agent:claude@ab12cd34. Defaults to the owner." },
      on_behalf_of: { type: "string", description: "The monitor's owner, when you are an agent acting at their access level. Only the owner may be named; the record keeps both names." },
