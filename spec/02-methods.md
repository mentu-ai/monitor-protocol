...
| `monitors/create` | the registration token, when the server requires one; open otherwise | `Monitor` minus server fields; `registration_token` may travel in the body. A server that requires one marks its monitors `attested`; a server that does not says so in every state it serves. | `Monitor` + emits `…configured{action:create}` | `INVALID_FILTER`, `UNKNOWN_VOCABULARY`, `DUPLICATE` |
